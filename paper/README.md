...
- `refs.bib`: BibTeX references. All entries verified against the
  canonical source (DGEB at ICLR 2025, AlphaFold DB 2024 in NAR,
  ProstT5 in NAR Genomics and Bioinformatics, Riziotis et al. 2025
  in The FEBS Journal).
...
NeurIPS workshop format) is not included here; download it from the
MLCB website and adapt before submission.

## Things to double-check before submission

1. **MLCB style file.** Swap the `\documentclass` and packages for the
   MLCB-supplied style.
2. **Page count.** Currently around 7 pages of content + 1 page refs.
   Check against the actual MLCB page limit (usually 8 incl. refs for
   a short paper).
3. **AI-tool disclosure.** The Acknowledgements mentions Claude as a
   coding partner. MLCB / NeurIPS allow this disclosure but check the
   2026 call-for-papers for any format requirements.
4. **Statistical reporting.** All p-values come from a Mann-Whitney U
