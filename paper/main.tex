...
($\text{NT v2--250M} = 0.506$). We show that a confidence-gated
...
of $0.680$. A per-query analysis shows that the queries our ensemble
...
benchmark. We also report negative results --- pocket-geometry features,
...
\emph{convergent} regime --- enzymes that catalyse the same reaction
...
on this task --- ESM2--3B --- reaches a weighted F1 of $0.265$. By
contrast, DGEB reports that several nucleotide-based models cross
$F1 = 0.50$ on the same task, e.g.\ NT v2--250M~\citep{dallatorre2024nt}
at $0.506$ and Evo~\citep{nguyen2024evo} at $0.446$. This inversion
--- DNA-level models outperforming AA-level models by nearly $2\times$
on a protein function classification task --- is itself notable, and
is the empirical anomaly motivating our analysis.
...
signal, structural-retrieval tools that operate on local 3D environment
--- such as Foldseek's 3Di alphabet
\citep{vankempen2024foldseek} --- should recover much of what
sequence-only models miss, without any task-specific training.

This paper makes the following empirical contributions.

\begin{itemize}
\item We report, to our knowledge, the first Foldseek score on DGEB
...
\item The same recipe transfers to DGEB EC Classification (an easier,
      ...
      ESM2--3B reference of $0.680$. Every configuration we tested in
      the ensemble sweep clears the baseline.
\item A per-query analysis on Convergent Enzymes shows that the queries
      our ensemble misses are statistically longer ($p = 0.02$) and
...
\item Documented negative results: amino-acid sequence features
      ...
      $\leq 0.073$; Nucleotide Transformer v1 2.5B embeddings in our
      pipeline score $0.008$ alone and reduce the ensemble F1 when
      added.
\end{itemize}

We do not contest the nucleotide-track ceiling on this benchmark. Our
...
We use the DGEB \texttt{convergent\_enzymes} dataset
...
either split we download the AlphaFold predicted structure
...
Our pipeline has three components.

\paragraph{Structural retrieval (Foldseek).}
We build a Foldseek database from the train set and run
\texttt{easy-search} with default parameters
(\texttt{-s 9.5 -e 10 --max-seqs 50}) and the standard 3Di scoring
matrix. For each test query, the top-1 hit's EC label is returned. We
attach Foldseek's per-hit alignment probability \texttt{prob} as a
confidence signal.

\paragraph{Amino-acid language-model fallbacks.}
We embed every protein with four LMs and treat each as a separate
classifier head. We use ESM2--35M, ESM2--150M, and ESM2--3B
...
truncated. For each LM we fit a regularised logistic regression
...
Table~\ref{tab:main} reports our best ensemble configuration on each
...
configuration we sweep clears the $0.680$ baseline, and the best
...
ESM2--3B alone (LR) & $0.188$ & our pipeline; DGEB reports $0.265$ \\
...
\begin{itemize}
\item Our reimplementation of ESM2--3B alone scores $0.188$, against
      $0.265$ reported by DGEB on the same model. We use a single
      ...
      different downstream protocol (we did not replicate their
      evaluation code, and they have not released per-query
      predictions). Our Foldseek + small-LM ensemble at $F1 = 0.267$
      exceeds the DGEB-reported number despite the underlying ESM2--3B
      component being weaker in our pipeline, because the ensemble is
...
\item Discrete motif counting on 3Di tokens --- whether linear k-mers,
      ...
      in our experiments. Foldseek's edge over exact-motif counting
...
Within our ensemble, the per-LM contributions are very different on
...
unsure --- their value is in their disagreement patterns rather than
in any single one being strong. On EC Classification the LMs are
...
Of the $400$ Convergent Enzymes test queries, our best ensemble
correctly predicts $126$ ($31.5\%$ accuracy; the weighted F1 of
$0.267$ accounts for class-imbalance reweighting). We compare the
distributions of several per-query properties across the
ensemble-correct and ensemble-incorrect subsets
(Table~\ref{tab:hard}). We test each marginal with a two-sided
Mann--Whitney $U$ test.
...
\item Foldseek's per-hit probability is by far the strongest signal of
      ...
      our confidence-gating strategy is well calibrated and that the
...
contrast against per-query NT v2 predictions, which we do not have
...
cavities per AlphaFold structure. We extracted the top-3 pockets'
...
We tested three motif-counting variants on the 3Di sequences of the
...
None approaches Foldseek's $F1 = 0.238$. We attribute this to
...
We attempted to engage the NA track by adding Nucleotide Transformer
...
\texttt{transformers} releases do not provide as defaults. We fell
back to NT v1 2.5B Multispecies, which loads cleanly. In our pipeline
...
architecture difference is large on this task --- DGEB does not
benchmark v1 on Convergent Enzymes; (b) our downstream protocol
under-reproduces DGEB's reported AA numbers too, so the gap between
our NT result and DGEB's is partly a downstream-protocol issue
...
tested clears $F1 = 0.45$. Our result on this task ($F1 = 0.267$) does
not contest this gap --- we work entirely on the AA track. Our
...
provides a property-based conjecture: queries our AA ensemble misses
...
much larger margin ($+0.051$ F1). This is not because EC is "less
...
\begin{itemize}
\item We evaluate on two DGEB tasks. Generalisation to the broader
      DGEB suite (operonic pairs, retrieval, phylogeny) requires
      pipeline changes our current code does not implement.
\item The Convergent Enzymes win margin is small ($+0.002$ F1) and
      not by itself a strong claim. We rely on the multi-task pattern
      and the ablation chain rather than the headline number.
\item Our reimplementation of ESM2--3B falls short of DGEB's published
      number for that model on this benchmark by $0.077$ F1. The gap is
      a downstream-protocol mismatch; we did not replicate DGEB's
      evaluation pipeline. Our ensemble does not depend on closing
      that gap.
\item We did not engage the NA track. The Phase 14 attempt with NT v1
...
exactly the ones with the GC-content and length signature we
...
course concluded. We thank Dr.\ Anna Green for pointing us at MLCB as
