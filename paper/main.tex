...
benchmark. Negative results are also reported: pocket-geometry
...
\item The same recipe transfers to DGEB EC Classification (an easier,
      ...
      ESM2-3B reference of $0.680$. Every configuration tested in the
      ensemble sweep clears the baseline.
\item A per-query analysis on Convergent Enzymes shows that the queries
      the ensemble misses are statistically longer ($p = 0.02$) and
...
Every ensemble configuration tested clears the DGEB ESM2-3B baseline
(dashed line at $0.680$); the best at $0.730$.}
\label{fig:ec}
\end{subfigure}
...
configuration in the sweep clears the $0.680$ baseline, and the best
...
\item The ESM2-3B reimplementation here falls short of DGEB's
      ...
      closing that gap.
\item The NA track is not engaged. The Phase 14 attempt with NT v1
