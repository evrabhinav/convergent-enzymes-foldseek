...
\begin{abstract}
The DGEB Convergent Enzymes Classification benchmark \citep{westroberts2024dgeb}
...
($\text{NT v2-250M} = 0.506$). This work shows that a confidence-gated
...
of $0.680$. A per-query analysis shows that the queries the ensemble
misses are statistically longer (Mann--Whitney $p = 0.02$ on CDS
length) and trend toward higher CDS GC content ($p = 0.09$), offering
a mechanistic conjecture for why DNA-level models close the AA-vs-NA
gap on this benchmark. Negative results are also reported: pocket-geometry
features, spatial 3Di motif counting, and a Nucleotide Transformer v1
ensembling attempt all underperform. Code, embeddings, and a fully
reproducible pipeline are public.\footnote{\url{https://github.com/evrabhinav/convergent-enzymes-foldseek}}
\end{abstract}
...
(DGEB) \citep{westroberts2024dgeb} includes a Convergent Enzymes Classification
...
empirical anomaly motivating the analysis presented here.

The Riziotis hypothesis \citep{riziotis2025paradigms} suggests that
...
signal, structural-retrieval tools that operate on the local 3D environment,
such as Foldseek's 3Di alphabet \citep{vankempen2024foldseek}, should
...
\item To the author's knowledge, this is the first reported Foldseek
      score on DGEB Convergent Enzymes Classification: $F1 = 0.238$
      (top-1) with no training and a single AlphaFold structure per
      protein.
...
\item The same recipe, extended to a 4-LM majority fallback for
      this task, transfers to DGEB EC Classification (an easier,
      homology-preserving task), reaching $F1 = 0.730$ versus the
      ESM2-3B reference of $0.680$. Every majority-fallback configuration
      in the $\tau$ sweep clears the baseline.
\item A per-query analysis on Convergent Enzymes shows that the
      queries the ensemble misses are statistically longer in CDS
      length (Mann--Whitney $p = 0.02$) and trend toward higher CDS GC
      content ($p = 0.09$). Both properties
...
This work does not contest the nucleotide-track ceiling on this
benchmark. The result characterises the AA side of the AA-vs-NA gap
precisely and offers a property-based hypothesis for what the DNA side
is exploiting.
...
\begin{tabular}{lrrrr}
\toprule
Task & Train / Test & DGEB AA-best & This work & $\Delta$ \\
\midrule
...
few-shot, homology removed). The proposed ensemble (red) ties the
DGEB AA-best baseline ESM2-3B (dashed line at $0.265$).}
\label{fig:ce}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
\centering
...
Every Foldseek-then-majority configuration in the $\tau$ sweep clears
the DGEB ESM2-3B baseline (dashed line at $0.680$); the best at
$0.730$.}
\label{fig:ec}
\end{subfigure}
...
the strongest amino-acid foundation-model baseline reported by DGEB
...
reaches $F1 = 0.238$. On EC Classification the gain is much larger
and qualitatively different: every Foldseek-then-4-LM-majority
configuration in the $\tau$ sweep clears the $0.680$ baseline (range
$0.721$--$0.730$), and the best configuration improves by $+0.050$
F1.
...
which AA-side models (the ensemble here and DGEB's) get wrong have a
quantifiably different DNA-level signature from the queries they get
right. A direct contrast against per-query NT v2 predictions, which
are not publicly available, would test this conjecture.
...
An attempt was made to engage the NA track by adding Nucleotide
Transformer predictions to the ensemble. NT v2
\citep{dallatorre2024nt}, the variant DGEB reports as $F1 = 0.506$ on
this task, failed to load under recent \texttt{transformers} versions
on Colab; its custom remote-code config exposes attributes
(\texttt{rope\_theta}, \texttt{is\_decoder}, \dots) that newer
\texttt{transformers} releases do not provide as defaults. The fallback
was NT v1 2.5B Multispecies, which loads cleanly. In this pipeline
(LogReg on mean-pooled embeddings) it scores $F1 = 0.008$ alone, and
adding it to the majority vote drops the ensemble from $F1 = 0.267$ to
$F1 = 0.264$. Two factors likely matter: (a) the v1$\leftrightarrow$v2
architecture difference is large on this task, and DGEB does not
benchmark v1 on Convergent Enzymes; (b) the downstream protocol used
here under-reproduces DGEB's reported AA numbers too, so the gap
between this NT result and DGEB's is partly a downstream-protocol
issue unrelated to the model itself. Resolving this would require
either the NT v2 compatibility issue or access to DGEB's downstream
code.
...
tested clears $F1 = 0.45$. The result on this task ($F1 = 0.267$) does
not contest this gap; this work operates entirely on the AA track. The
...
provides a property-based conjecture: queries the AA ensemble misses
...
\begin{itemize}
\item Only two DGEB tasks are evaluated. Generalisation to the broader
      DGEB suite (operonic pairs, retrieval, phylogeny) requires
      pipeline changes that the current code does not implement.
\item The Convergent Enzymes win margin is small ($+0.002$ F1) and
      not by itself a strong claim. The argument rests on the
      multi-task pattern and the ablation chain rather than the
      headline number.
\item The ESM2-3B reimplementation here falls short of DGEB's
      published number for that model on this benchmark by $0.077$ F1.
      The gap is a downstream-protocol mismatch; DGEB's evaluation
      pipeline was not replicated. The ensemble does not depend on
      closing that gap.
\item The NA track is not engaged. The NT v1 ensembling attempt is a
      negative result, not a contradiction of DGEB's NT v2 result.
\end{itemize}
...
exactly the ones with the GC-content and length signature identified
here? If yes, that would localise the AA-vs-NA gap to a small set of
DNA-level properties that could in principle be approximated by
